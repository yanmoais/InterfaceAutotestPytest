...
#查询还款订单列表
@allure.epic("查询还款订单列表")
@allure.feature("查询还款订单列表")
@allure.title("查询还款订单列表")
@allure.story("查询还款订单列表")
...
def test_repay_list():
    ...
    with allure.step("登录"):
        ...
        time.sleep(1)
        #2.查询还款订单列表
        response_data2 = Core_tyh_app().test_repay_list(headers=token)
        assert response_data2['data'] != null
        logging.info(f"用户还款订单列表的数据为：======{response_data2}")

...
def test_runlou_loan_success_api_flow():
...
@allure.severity(allure.severity_level.CRITICAL)
@pytest.mark.skip()
def test_runlou_repay_d0_success_api_flow():
    with allure.step("数据初始化"):
        # 核心api的基类
        api = core_api_flow_api()
        # 加解密基类
...
@allure.severity(allure.severity_level.CRITICAL)
@pytest.mark.skip()
def test_runlou_repay_success_api_flow():
    with allure.step("数据初始化"):
        # 核心api的基类
        api = core_api_flow_api()
        # 加解密基类
        ...
        # 修改对应的缩写或其他标志：润楼  ==  RL
        loanApplyNo = get_req_seq_no("RL")
        repayApplyNo = get_credit_apply_no("RL")
        # 资金方，修改成对应需要放款的资金方funds_code
        funds_code = "FR_RUN_LOU"
        # 渠道方，修改成对应需要走的渠道方channel_code
        channel_code = "ICE_ZLSK_36"

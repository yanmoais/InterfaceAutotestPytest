...
def test_zx_loan_success_api_flow(get_channel,get_loan_perid):
...
@allure.story("360沙盒渠道-振兴资方授信案例-API全流程")
@allure.severity(allure.severity_level.CRITICAL)
def test_zx_credit_success_api_flow(get_channel,get_loan_perid):
    with allure.step("数据初始化"):
        # 核心api的基类
        api = core_api_flow_api()
        # 加解密基类
...
@allure.story("360沙盒渠道-振兴资方授信案例-API全流程")
@allure.severity(allure.severity_level.CRITICAL)
def test_zx_band_card_success_api_flow(get_channel,get_loan_perid):
    with allure.step("数据初始化"):
        # 核心api的基类
        api = core_api_flow_api()
        # 加解密基类

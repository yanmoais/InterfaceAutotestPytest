...
def test_jmx_loan_success_api_flow(get_channel):
    ...
    with allure.step("更新授信相关表为金美信消金资方"):
...
@allure.story("360沙盒渠道-金美信消金资方案例-API全流程")
@allure.severity(allure.severity_level.CRITICAL)
def test_jmx_credit_success_api_flow(get_channel):
    with allure.step("数据初始化"):
        # 核心api的基类
        api = core_api_flow_api()
        # 加解密基类
...
@allure.story("360沙盒渠道-金美信消金资方案例-API全流程")
@allure.severity(allure.severity_level.CRITICAL)
def test_jmx_band_card_success_api_flow(get_channel):
    with allure.step("数据初始化"):
        # 核心api的基类
        api = core_api_flow_api()
        # 加解密基类
...
# @allure.severity(allure.severity_level.CRITICAL)
# def test_jmx_repay_d0_success_api_flow():
#     with allure.step("数据初始化"):
#         # 核心api的基类
#         api = core_api_flow_api()
#         # 加解密基类
...
def test_jmx_repay_success_api_flow():
    with allure.step("数据初始化"):
        ...
        select_db = Select_Sql_Result()
        # 身份证生成
        id_no, birthday = '440113199409245653', '1994-09-24'
        # 用户名生成
        user_name = '龚欢元'
        # 宝付规则卡号生成
        bank_card_no = '6217201741749264380'
        # custID生成
        user_id = 'SUR2398793965'
        # 绑卡ID生成
        certificationApplyNo = get_api_bk_id()
        # 日志基类
        logging = Logger().init_logger()
        ...
        # 获取风控加白了的手机号，读取本地txt文件
        mobile_no = '15935900802'
        # 修改对应的缩写或其他标志：金美信消金  ==  JMX
        loanApplyNo = 'SLN7682527784'
        repayApplyNo = get_credit_apply_no("CYBF")
        # 资金方，修改成对应需要放款的资金方funds_code
        funds_code = "JMX"
        # 渠道方，修改成对应需要走的渠道方channel_code
        channel_code = "TYH_RPPLUS"
        # 借款金额
        loan_amt = "2000"
        # 借款期数
        reqPeriods = "12"
    ...
    with allure.step("查询相关表取数"):
        partnerLoanNo = "1899631735755386880"
        bankCode = "0004"
        idCardNo = "440113199409245653"
        userMobile = "15935900802"
        sharePayAgreementId = "1202503121114579340005833387"

...
def test_mshang_loan_success_api_flow(get_channel,get_loan_perid):
...
@allure.severity(allure.severity_level.CRITICAL)
@pytest.mark.skip()
def test_mshang_repay_d0_success_api_flow(get_channel,get_loan_perid):
    with allure.step("数据初始化"):
        api = core_api_flow_api()
        enc = encrypt_decrypt()
        credit_apply_no = get_credit_apply_no()
...
@allure.severity(allure.severity_level.CRITICAL)
@pytest.mark.skip()
def test_mshang_repay_success_api_flow(get_channel,get_loan_perid):
    with allure.step("数据初始化"):
        # 核心api的基类
        api = core_api_flow_api()
        # 加解密基类

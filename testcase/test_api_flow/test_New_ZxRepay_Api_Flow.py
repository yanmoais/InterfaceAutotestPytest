...
@allure.epic("360沙盒渠道-新振兴资方-API全流程")
@allure.feature("360沙盒渠道-授信模块-API全流程")
@allure.title("360沙盒渠道-借款成功-API全流程")
@allure.story("360沙盒渠道-新振兴资方授信案例-API全流程")
@allure.severity(allure.severity_level.CRITICAL)
def test_new_zx_loan_success_api_flow(get_channel,get_loan_perid):
    ...
    with allure.step("更新授信相关表为新振兴资方"):
    ...
    with allure.step("生成测试结果"):
        ...
        test_result = {
            ...
            "loanPeriod": reqPeriods  # 新增：添加借款期数字段
        }
        # 输出测试结果 - 确保只输出一行
...
@allure.story("360沙盒渠道-新振兴资方授信案例-API全流程")
@allure.severity(allure.severity_level.CRITICAL)
def test_new_zx_credit_success_api_flow(get_channel,get_loan_perid):
    with allure.step("数据初始化"):
        # 核心api的基类
        api = core_api_flow_api()
        # 加解密基类
...
@allure.story("360沙盒渠道-新振兴资方授信案例-API全流程")
@allure.severity(allure.severity_level.CRITICAL)
def test_new_zx_band_card_success_api_flow(get_channel,get_loan_perid):
    with allure.step("数据初始化"):
        # 核心api的基类
        api = core_api_flow_api()
        # 加解密基类
...
@allure.story("360沙盒渠道-新振兴资方还款案例-API全流程")
@allure.severity(allure.severity_level.CRITICAL)
@pytest.mark.skip()
def test_zx_repay_d0_success_api_flow(get_channel,get_loan_perid):
    with allure.step("数据初始化"):
        # 核心api的基类
        api = core_api_flow_api()
        # 加解密基类

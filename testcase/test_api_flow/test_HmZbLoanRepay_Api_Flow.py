...
def test_zb_band_card_success_api_flow(get_channel, get_loan_perid):
    ...
    if resp in (False, None):
        ...
        with allure.step("绑卡申请"):
            # 请求鉴权数据
            bk_jq_need_encry_data = {"userId": user_id, "certificationApplyNo": certificationApplyNo,
                                     "bankCode": "0003",
                                     "idCardNo": id_no, "userMobile": mobile_no, "userName": user_name,
                                     "bankCardNo": bank_card_no, "registerMobile": mobile_no,
                                     "agreementTime": apply_time
                                     }
...
@allure.story("360沙盒渠道-哈密众邦资方授信案例-API全流程")
@allure.severity(allure.severity_level.CRITICAL)
def test_zb_credit_success_api_flow(get_channel, get_loan_perid):
    with allure.step("数据初始化"):
        api = core_api_flow_api()
        enc = encrypt_decrypt()
        credit_apply_no = get_credit_apply_no()
...
def test_zb_loan_success_api_flow(get_channel, get_loan_perid):
    ...
    if resp in (False, None):
        ...
        with allure.step("轮询执行JOB借款成功"):
            ...
            jk_success_resp = loop_result().loop_api_flow_loan_result(loan_query_need_encry_data, loanApplyNo,
                                                                      channel_code)
    ...
    if resp in (False, None):
        ...
        if jk_success_resp in (False, None):
            ...
            with allure.step("轮询执行D0批扣还款成功"):
                # 执行api侧D0批扣任务
                time.sleep(1)
                execute_xxl_job().hmzb_d0_repay_job()
...
@allure.story("360沙盒渠道-哈密众邦资方还款案例-API全流程")
@allure.severity(allure.severity_level.CRITICAL)
def test_zb_due_repay_success_api_flow(get_channel, get_loan_perid):
    with allure.step("数据初始化"):
        # 核心api的基类
        api = core_api_flow_api()
        # 加解密基类
    ...
    if resp is not False:
        ...
        with allure.step("主动还款"):
            ...
            repay_need_encry_data = {"userId": user_id, "repayApplyNo": repayApplyNo,
                                     ...
                                     "bankCardInfo": {"bankCode": "0003", "idCardNo": id_no,
        ...
        with allure.step("轮询还款状态"):
            loop_result().loop_api_flow_repay_result(loanApplyNo, 'due_repay')
        ...
        logging.info(f"该笔申请未授信通过，请重新尝试！")

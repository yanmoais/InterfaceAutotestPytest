...
@allure.epic("润楼资方")
@allure.feature("授信模块")
@allure.story("润楼资方授信放款案例")
@allure.title("放款成功")
def test_runlou_loan_success():
    ...
    with allure.step("发起授信"):
        ...
        # 1.授信申请返回数据解密
        decry_data = encrypt_decrypt().param_decrys_by_channel(sxsq_resp, 'runLou')
        logging.info(f"解密后的授信申请返回数据为：======{decry_data}")

    with allure.step("授信状态查询"):
        # 2.授信状态查询加密
        zt_need_ency_data = {"apiKey": "RLTEST","params":json_dumps_cn({"loanseqno":loan_sqe_no}),"requestNo": req_no}
        # 加密数据
        logging.info(f"需要加密的授信状态查询数据为：======{zt_need_ency_data}")
        sxzt_encry = encrypt_decrypt().param_encry_by_channel(zt_need_ency_data, 'runLou')
        logging.info(f"加密后的授信状态查询数据为：======{sxzt_encry}")
        # 轮训读取判断查询结果，为"授信通过"则跳出
        # 2.授信状态查询请求
        loop_result().loop_sxcx_result(sxzt_encry, 'runLou')

    with allure.step("发起通联绑卡"):
        # 3.签约申请加密
        qybk_ency_data = {"apiKey": "RLTEST","params":json_dumps_cn({"seqno":bk_no,"loanseqno":loan_sqe_no,"id_no":id_no,"user_name":user_name,"mobile_no":mobile_no,"bankCode":"0105","cardNo":acct_no,"bankName":bank_name,"registerPhone":mobile_no}),"requestNo": req_no}
        # 加密数据
        logging.info(f"需要加密的签约申请数据为：======{qybk_ency_data}")
        qybk_encry = encrypt_decrypt().param_encry_by_channel(qybk_ency_data, 'runLou')
    ...
    with allure.step("绑卡签约确认"):
        ...
        qyqr_encry = encrypt_decrypt().param_encry_by_channel(qyqr_encry_data,'runLou')
    ...
    with allure.step("还款计划查询"):
        ...
        # 7.还款计划查询返回数据解密
        hkjh_decry = encrypt_decrypt().param_decrys_by_channel(hkjh_resp, 'runLou')

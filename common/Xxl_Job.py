...
class xxlJob(object):
    def __init__(self):
        ...
        self.xxl_job_host = XXL_JOB_HOST
        self.logging = Logger().init_logger()

    # 登录XXL-JOB
    def login_xxl_job(self):
        uri = f'{XXL_JOB_HOST}xxl-job-admin/login'
        headers = {
        ...
        data = requests.post(uri, data=params, cookies=cookies, headers=headers, proxies=proxies, verify=False,
    ...
    # 执行对应的JOB任务
    def trigger_xxl_job(self, id, executorParam):
        self.login_xxl_job()
        uri = f'{XXL_JOB_HOST}xxl-job-admin/jobinfo/trigger'
        headers = {
